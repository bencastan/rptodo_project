...
def init(
    db_path: str = typer.Option(
        ...
        "--db-path",
        "-db",
        prompt="to-do dtatabse location?",
    ),
) -> None:
    ...
    if db_init_error:
        type.secho(
            f'Creating datbase failed with "{ERRORS[db_init_error]}"',
            fg=typer.colors.RED
        )
        raise typer.Exit(1)
    else:
...
def get_todoer() -> rptodo.Todoer:
    if config.CONFIG_FILE_PATH.exists():
        db_path = database.getdatabase_path(config.CONFIG_FILE_PATH)
    else:
        typer.secho(
...
def set_done(todo_id: int = typer.Argument(...)) -> None:
    """Complete a to-do by setting tit as done using its TODO_ID."""
    toder = get_todoer()
...
def remove_all(
    force: bool = typer.Option(
        ...
        help="Force deletion of all to-dos without confirmation.",
    ),
) -> None:
    """Remove all to-dos"""
    todoer = get_todoer()
